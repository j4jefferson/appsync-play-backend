...
describe('Given user A and user B', () => {
	...
	describe('When user B follows user A', () => {
		...
		it("Adds User A's tweets to User B's timeline", async () => {
			await then.tweet_exists_in_the_TimelinesTable(userB.username, userAsTweet1.id)
			await then.tweet_exists_in_the_TimelinesTable(userB.username, userAsTweet2.id)
		})
	})
	...
	describe('When user B unfollows user A', () => {
		...
		it("Removes User A's tweets from User B's timeline", async () => {
			await then.tweet_does_not_exists_in_the_TimelinesTable(
				userB.username,
				userAsTweet1.id
			)
			await then.tweet_does_not_exists_in_the_TimelinesTable(
