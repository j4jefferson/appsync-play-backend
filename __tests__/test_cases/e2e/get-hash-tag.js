...
const { HashTagModes, TweetTypes } = require('../../../lib/constants.js')
...
describe('Given an authenticated user', () => {
	...
	beforeAll(async () => {
		...
		when.a_user_calls_editMyProfile(userA, {
	...
	it('A user can find himself when he gets the hash tag with PEOPLE', async () => {
		await retry(
			async () => {
	...
	describe('When a user sends a tweet', () => {
		...
		it('A user can find his tweet when he gets the hash tag with LATEST', async () => {
			await retry(
				async () => {
					const { results, nextToken } = await when.a_user_calls_getHashTag(
						userA,
		...
		describe('When a user replies to the tweet', () => {
			let reply
			...
			it('A user can find his reply when he gets hash tag for LATEST', async () => {
				await retry(
					async () => {
						const { results, nextToken } = await when.a_user_calls_getHashTag(
							userA,
